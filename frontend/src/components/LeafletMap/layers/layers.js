...
export const layerFactory = {
  ...
  targets: () =>
    L.geoJSON(null, {
      pointToLayer: (feature, latlng) =>
        L.circleMarker(latlng, { color: "blue", radius: 5 }),
      onEachFeature: (feature, layer) => {
        layer.bindPopup(
          `<b>Detected Target</b><br>ID: ${feature.properties.id}`
        );
      },
    }),
    ...
    L.geoJSON(null, {
      ...
      onEachFeature: (feature, layer) => {
        ...
        layer.on("click", (e) => {
          ...
          moveFeature(feature, "approved_targets", "removed_targets");
        });
      },
    }),
  ...
  depot_points: () =>
    L.geoJSON(null, {
      ...
      onEachFeature: (feature, layer) => {
        layer.bindPopup(
          `<b>Depot</b><br>ID: ${feature.properties.id}<br>Radius: ${
            feature.properties.depot_radius || 500
          }m`
        );
      },
    }),
  ...
  default: () =>
    L.geoJSON(null, {
      pointToLayer: (feature, latlng) =>
        L.circleMarker(latlng, { color: "gray", radius: 5 }),
      onEachFeature: (feature, layer) => {
        layer.bindPopup(
          `<b>Unknown Layer</b><br>ID: ${feature.properties?.id || "N/A"}`
        );
      },
    }),
};
...
export function updateLayerData(layerKey, geojson) {
  ...
  if (mapLayers[layerKey]) {
    ...
    if (mapLayers["region_contour"]) {
      mapLayers["region_contour"].bringToBack(); // Added this for the audit page
      // Trying to avoid editing contour, don't drag it.
    }
  } else {
...
export function moveFeature(feature, fromLayer, toLayer) {
  if (!mapLayers[fromLayer] || !mapLayers[toLayer]) return;
...
// Get the properties of a given layer or layer group's sub-layers
export function getLayerProperties(layerName) {
  if (!mapLayers[layerName]) return {}; // Layer not found

  let layerProperties = {};
  mapLayers[layerName].eachLayer((layer) => {
    if (layer.feature) {
      layerProperties = { ...layer.feature.properties };
      return layerProperties;
    }
  });

  return layerProperties; // Default if none found
}
